...
build/windchimes.linux-amd64.exe: $(GO_FILES)
	GOOS=linux   GOARCH=amd64       go build -o $@ .
build/windchimes.linux-386.exe: $(GO_FILES)
	GOOS=linux   GOARCH=386         go build -o $@ .
build/windchimes.linux-arm-7.exe: $(GO_FILES)
	GOOS=linux   GOARCH=arm GOARM=7 go build -o $@ .
build/windchimes.linux-arm64.exe: $(GO_FILES)
	GOOS=linux   GOARCH=arm64       go build -o $@ .
build/windchimes.win-amd64.exe: $(GO_FILES)
	GOOS=windows GOARCH=amd64       go build -o $@ .
build/windchimes.win-386.exe: $(GO_FILES)
	GOOS=windows GOARCH=386         go build -o $@ .
build/windchimes.mac-arm64.exe: $(GO_FILES)
	GOOS=darwin   GOARCH=arm64      go build -o $@ .
build/windchimes.mac-amd64.exe: $(GO_FILES)
